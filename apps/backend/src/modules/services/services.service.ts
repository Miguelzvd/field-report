...
import { ServiceSelect, ChecklistItemSelect, PhotoSelect } from "../../db/schema"
...
export async function updateService(
  id: string,
  userId: string,
  data: Partial<{ type: ServiceType; status: "open" | "finished"; finishedAt: Date | null }>
): Promise<ServiceSelect> {
  const existing = await servicesRepository.findServiceByIdAndUserId(id, userId)
